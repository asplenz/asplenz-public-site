          ...
          <h2 className="font-serif text-2xl md:text-4xl text-[var(--text-primary)] mb-4 flex flex-nowrap items-center gap-x-3 md:gap-x-5">
            {ix.levels.title.replace(/\.$/, '').split('. ').map((word: string, i: number) => (
              <span key={i} className="flex items-center gap-2">
                ...
                {i === 0 && (
                  <svg className="hidden md:block w-7 h-7 text-[var(--accent)] shrink-0" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="md:hidden space-y-3">
            {ix.comparison.rows.map((row: any, i: number) => (
              <div key={i} className="border border-[var(--border)] rounded-xl p-4 bg-[var(--bg-card)]">
                <p className="font-semibold text-[var(--text-primary)] text-sm mb-3">{row.criterion}</p>
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div>
                    <p className="text-[var(--text-muted)] font-mono mb-1">{ix.comparison.col1}</p>
                    <p className="text-[var(--text-secondary)]">{row.before}</p>
                  </div>
                  <div>
                    <p className="text-[var(--accent)] font-mono font-medium mb-1">✦ {ix.comparison.col2}</p>
                    <p className="text-[var(--text-secondary)]">{row.after}</p>
                  </div>
                </div>
              </div>
            ))}
