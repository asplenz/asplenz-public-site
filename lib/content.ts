...
export const content = {
  en: {
    ...
    nav: {
      ...
      aiAgents: 'AI Agents',
      ciVerifier: 'CI Compliance Check',
      gettingStarted: 'Getting Started',
      concepts: 'Concepts',
      integrations: 'Integrations',
      docsAiAgents: 'AI Agents & MCP',
    ...
    index: {
      hero: {
        ...
        headlineItalic: 'Your agents don\'t know them.',
      ...
      heroCard: {
        title: 'Agent - Finance / Risk',
        steps: [
          { icon: '📋', label: 'Invariants loaded', text: 'scope: "finance" / ns: "risk"', badge: 'Max 5% per issuer', badgeColor: 'blue' },
          { icon: '📐', label: 'Rules loaded', text: 'scope: "finance" / ns: "pricing"', badge: 'EUR mid-swap benchmark — MANDATORY', badgeColor: 'blue' },
          { icon: '🔍', label: 'Exposure check', text: 'Current: 4.2% + requested +2% | Result: 6.2% > limit 5%', badge: '', badgeColor: 'orange' },
          { icon: '⚠️', label: 'Agent decision', text: 'Purchase not recommended', badge: 'finance/risk invariant consulted', badgeColor: 'red' },
          { icon: '✓', label: 'Choice documented', text: '', badge: 'dec-d2c9e8e8fa1b', badgeColor: 'green' },
        ],
      },
      problem: {
        tag: 'The problem',
      ...
      levels: {
        ...
        level1: {
          ...
          title: 'Knowledge surfaces implicit decisions from your repos, PRs, and docs.',
          desc: 'Knowledge analyses your repos and generates typed drafts: candidate invariants, rules, and decisions. Nothing enters the registry without human validation.',
          features: [
          ...
          quote: 'Tacit knowledge doesn\'t disappear — it enters the registry.',
        ...
        level2: {
          ...
          concepts: {
            cards: [
              ...
              { n: 2, badge: 'Rules', badgeColor: 'blue', title: 'Active directives', desc: 'Mandatory or advisory operational conventions. Versioned: when a rule evolves, the full history is preserved.', example: 'e.g. "Use EUR mid-swap curve for bond pricing."' },
              { n: 3, badge: 'Decisions', badgeColor: 'green', title: 'Documented choices', desc: 'Immutable records of significant choices — with context, reasoning, author, and timestamp. The rationale behind every rule.', example: 'e.g. "EUR mid-swap adopted as bond pricing benchmark — better liquidity for corporate spreads. Approved by head of pricing, March 2024."' },
              { n: 4, badge: 'Overrides', badgeColor: 'orange', title: 'Formal exceptions', desc: 'A documented exception to a rule or invariant — with named approver, justification, conditions, and expiry. Not a workaround: a governed exception.', example: 'e.g. "30-day exception — exposure limit raised to 7% for TotalEnergies, approved by CIO."' },
            ],
          },
          scopes: {
            tag: 'Scopes & Namespaces',
            ...
            code: 'finance\n ├─ pricing      → benchmarks, curves\n ├─ risk         → limits, VaR, exposure\n ├─ compliance   → regulatory, reporting\n └─ operations   → validations, process\n\nengineering\n ├─ security      → secrets, access, CVE\n ├─ architecture  → patterns, decisions\n └─ cicd          → deployment, gates',
          },
          graphSection: {
            ...
            relations: [
              { from: 'Rule', rel: 'DEPENDS_ON', to: 'which decision justifies this directive?' },
              { from: 'Override', rel: 'OVERRIDES', to: 'which exception suspends this directive, when and why?' },
              { from: 'Decision', rel: 'REPLACES', to: 'does this choice replace a previous one?' },
              { from: 'Rule', rel: 'IN_TENSION_WITH', to: 'do these two directives contradict each other?' },
            ],
          },
          features: [
          ...
          quote: 'Rules are no longer in someone\'s head — they\'re in the registry, queryable, enforceable.',
        },
        level3: {
          badge: 'Level 3 — Apply',
          tag: 'Agentic application',
          title: 'Your agents query Knowledge before acting',
          desc: 'A minimal system prompt tells the agent to query Knowledge before acting. The rules themselves are fetched on demand — no prompt overload, no loss in long context.',
          features: [
          ...
          quote: 'Governance is not a suggestion — it\'s a registry the agent queries before every significant action.',
          hooks: {
            ...
            rows: [
              { hook: 'UserPromptSubmit', trigger: 'Every user prompt', action: 'Injects fresh list_invariants + list_rules', effect: 'Agent knows the rules at every interaction — not just at session start' },
              { hook: 'PostToolUse (async)', trigger: 'After each tool call', action: 'Calls record_reference automatically', effect: 'Traceability is deterministic — LLM-independent' },
              { hook: 'Stop', trigger: 'Session end', action: 'Calls record to document the final choice', effect: 'Every session produces a structured decision' },
            ],
            result: 'Governance is not a suggestion in the system prompt — it\'s a mechanism injected into the agent runtime.',
          },
          flow: 'User sends a prompt\n    │\n    ▼\nHook UserPromptSubmit\n    → list_invariants(scope) — fresh constraints\n    → list_rules(scope) — fresh directives\n    │\n    ▼\nAgent plans its action\n    │\n    ├── Blocked by invariant → STOP, report\n    ├── Requires approval → REQUEST, WAIT\n    └── Allowed → EXECUTE\n    │\n    ▼\nHook PostToolUse (async)\n    → record_reference(entry_id, status="followed")\n    │\n    ▼\nHook Stop\n    → record(decision, context, reasoning)\n    │\n    ▼\nComplete trail: rules consulted → action executed → decision documented',
          integrations: [
            { icon: '🤖', name: 'Claude (MCP)', desc: 'Claude Teams & Enterprise', level: 'Hooks + MCP — full enforcement' },
            { icon: '💻', name: 'Claude Code', desc: 'Autonomous coding agent', level: 'Native hooks — enforcement by default' },
            { icon: '⚙️', name: 'GitHub Actions', desc: 'CI/CD — Knowledge Verifier', level: 'Blocking gate — no merge without compliance' },
            { icon: '🔌', name: 'REST API', desc: 'Any LLM, any framework', level: 'API consultation — enforcement to integrate' },
          ],
          code: '# 1. Query invariants — targeted scope + namespace\ninv = knowledge.list_invariants(\n    scope="finance", namespace="risk"\n)\n\n# 2. Query pricing rules\nrules = knowledge.list_rules(\n    scope="finance", namespace="pricing"\n)\n\n# 3. Document the choice with context and reasoning\nknowledge.record(\n    scope="finance",\n    decision="TotalEnergies purchase not recommended",\n    context="Request +2%, result 6.2%",\n    reasoning="Invariant finance/risk max 5% consulted"\n)\n\n# 4. Trace the reference to the invariant\nknowledge.record_reference(\n    entry_id=inv_id,\n    status="followed",\n    context_type="agent_session"\n)',
        },
      },
      useCases: {
        tag: 'Use cases',
        ...
        items: [
          { type: 'Autonomous agent', icon: '🏦', title: 'Finance & Trading', desc: 'Exposure limits, compliance rules, pricing — every decision linked to the rule that enforced it.' },
          { type: 'Autonomous agent', icon: '💻', title: 'Coding agents', desc: 'Claude Code and your CI/CD pipelines share the same architecture and security registry.' },
          { type: 'Autonomous agent', icon: '⚙️', title: 'CI/CD pipelines', desc: 'No merge without compliance — the Verifier blocks non-compliant PRs automatically.' },
          { type: 'Autonomous agent', icon: '💬', title: 'Conversational assistants', desc: 'Communication, confidentiality, and compliance rules enforced via MCP — no prompt to maintain.' },
          { type: 'Human copilot', icon: '🏥', title: 'Healthcare & Pharma', desc: 'Medical invariants enforced on the copilot. Final decision stays human — Knowledge traces what was consulted.' },
          { type: 'Human copilot', icon: '⚖️', title: 'Legal & Contractual', desc: 'Mandatory clauses, liability caps, approval levels — compliance traced at every interaction.' },
        ],
      },
      comparison: {
        tag: 'Why Knowledge',
        ...
        col2: 'Knowledge',
        rows: [
          { criterion: 'Updates', before: 'Manual, per instance', after: 'One rule changed, all agents updated' },
          { criterion: 'Enforcement', before: 'Forgettable suggestion', after: 'Injected hook — LLM-independent' },
          { criterion: 'Contextual memory', before: 'Forgotten in long sessions', after: 'Re-injected at every prompt' },
          { criterion: 'Exceptions', before: 'No formal mechanism', after: 'Override documented, approved, expirable' },
          { criterion: 'Compliance proof', before: 'Declared intent', after: 'Traced and auditable execution' },
        ],
      },
      aiAct: {
        tag: 'Compliance',
        ...
        cards: [
          { article: 'Art. 9', title: 'Risk management', desc: 'Codified invariants. Distill detects obsolescence.' },
          { article: 'Art. 11', title: 'Technical documentation', desc: 'Immutable decisions with context and reasoning.' },
          { article: 'Art. 12', title: 'Record-keeping', desc: 'Automatic logging of every agent interaction.' },
          { article: 'Art. 13', title: 'Transparency', desc: 'Versioned registry, consultable at any point in time.' },
          { article: 'Art. 14', title: 'Human oversight', desc: 'Approvals, formal overrides, roles and permissions.' },
          { article: 'Art. 15', title: 'Robustness', desc: 'Enforcement via hooks — LLM-independent.' },
        ],
        cta: 'Learn more about AI Act compliance',
        ctaHref: '/compliance/ai-act',
      },
      integrations: {
        tag: 'Integrations',
        title: 'Ready in a few lines',
        sub: 'Knowledge integrates via MCP or REST API into any LLM or AI agent.',
        items: [
          { icon: '🤖', name: 'Claude (MCP)', desc: 'Claude Teams & Enterprise' },
          { icon: '💻', name: 'Claude Code', desc: 'Autonomous coding agent' },
          { icon: '⚙️', name: 'GitHub Actions', desc: 'CI/CD pipelines' },
          { icon: '🔌', name: 'REST API', desc: 'Any LLM, any framework' },
        ],
        code: '# 1. Query invariants - targeted scope + namespace\ninv = knowledge.list_invariants(\n    scope="finance", namespace="risk"\n)\n\n# 2. Query pricing rules\nrules = knowledge.list_rules(\n    scope="finance", namespace="pricing"\n)\n\n# 3. Document the choice with context and reasoning\nknowledge.record(\n    scope="finance",\n    decision="TotalEnergies purchase not recommended",\n    context="Request +2%, result 6.2%",\n    reasoning="Invariant finance/risk max 5% consulted"\n)\n\n# 4. Trace the reference to the invariant\nknowledge.record_reference(\n    entry_id=inv_id,\n    status="followed",\n    context_type="agent_session"\n)',
      },
      pricing: {
        tag: 'Pricing',
        title: 'Connect your agent on day 1.',
        sub: 'The registry is accessible from the free plan. Extraction is a feature, not a tier.',
        tiers: [
          {
            name: 'Starter',
            tagline: 'Free',
            price: 'Free',
            desc: 'The registry from day one',
            features: [
          ...
          },
          {
            ...
            tagline: '€299/month',
            price: '€299/month',
            desc: 'Scale across your organization',
          ...
          },
          {
            ...
            tagline: 'Custom',
            price: 'Custom',
            desc: 'End-to-end governance at scale',
      ...
      observability: {
        ...
        sub: 'Coming soon — an agentic compliance dashboard: followed/diverged rate, never-queried rules, Distill obsolescence signals.',
      },
      ctaFinal: {
        tag: 'Ready to start?',
        ...
        secondaryHref: '/knowledge/docs',
      },
      // Legacy keys kept for compatibility
      shift: {
        title: 'AI velocity changed. Governance didn\'t.',
        intro: 'AI agents now modify systems at machine speed. They introduce dependencies, modify APIs, refactor architecture, deploy infrastructure, price instruments, analyze data, execute workflows.',
        intro2: 'But they don\'t know your architectural decisions, your compliance constraints, your risk thresholds, which exceptions were approved, or what must never change.',
        conclusion: 'Without a declared decision layer, agents guess.',
      },
      knowledge: {
        title: 'Knowledge: the normative layer your agents query before acting',
        intro: 'Knowledge is a declarative system of record for operational decisions. Four object types:',
        entities: [
          { name: 'Invariants', description: 'constraints that must never be violated' },
          { name: 'Rules', description: 'directives that guide implementation and operations' },
          { name: 'Decisions', description: 'documented choices with reasoning' },
          { name: 'Overrides', description: 'governed, time-bound exceptions with rationale and approver' },
        ],
        after1: 'Any agent calls `knowledge.resolve()` for its scope and receives the applicable rules, active overrides, detected conflicts, and the full resolution chain. Before it acts, not after.',
        after2: 'Knowledge does not block execution. It exposes the applicable decision state. Your ecosystem decides what to do with it.',
      },
      mcpNative: {
        title: 'MCP-native. Dynamic by design.',
        body: 'Agents call Knowledge on demand via MCP (Model Context Protocol).',
        body2: 'Not injected into every prompt. No token overhead. No bloated system prompts. Six dedicated tools, called only when the agent needs context.',
        examples: 'An engineering agent calls `resolve()` before coding. A compliance agent calls `check()` before processing a case. A pricing agent calls `query()` to know which model to apply. Same registry, same protocol, any workflow.',
      },
      scopeResolution: {
        title: 'Scope and resolution',
        intro: 'Context resolves deterministically.',
        code: 'engineering\n ├─ payments\n │    ├─ pci\n │    ├─ fraud\n ├─ platform\n ├─ data\n\nfinance\n ├─ credit\n │    ├─ investment-grade\n ├─ pricing\n ├─ risk',
        after: 'Each agent maps to a single path. Resolution flows from most specific to most global. When an agent queries `engineering/payments/pci` or `finance/credit/investment-grade`, Knowledge returns only the rules applicable to that path.',
        closing: 'No ambiguity. No hidden precedence. No duplication.',
      },
      whatItLooksLike: {
        title: 'This is what it looks like',
        examples: [
          {
            intro: 'An engineering agent opens PR #847. Instead of coding blind, it consulted Knowledge first.',
            header: 'PR Summary - Generated by AI',
            what: 'Implements the new payment notification service.',
            decisionsTitle: 'Design decisions',
            decisions: [
              { n: 1, title: 'gRPC for inter-service communication', ref: 'Rule ENG-R-042', detail: '"All new services use gRPC"', link: 'knowledge://engineering/rules/ENG-R-042' },
              { n: 2, title: 'PostgreSQL for transaction storage', ref: 'Decision ENG-D-118', detail: '"PostgreSQL over MongoDB for any service requiring ACID transactions"', link: 'knowledge://engineering/decisions/ENG-D-118' },
              { n: 3, title: 'No AGPL dependencies used', ref: 'Invariant ENG-I-003', detail: '', link: 'knowledge://engineering/invariants/ENG-I-003' },
            ],
            tradeoffsTitle: 'Trade-offs',
            tradeoffs: [
              { title: 'Used REST instead of gRPC for notifications (latency < 10ms)', ref: 'Override ENG-R-042', detail: 'approved by @cto', link: 'knowledge://engineering/overrides/ENG-O-047' },
            ],
          },
          {
            intro: 'A compliance agent processes a loan application. It called Knowledge first.',
            header: 'Agent action log',
            what: 'Loan application #2891 - SME credit line',
            decisionsTitle: 'Verified before processing:',
            decisions: [
              { n: 1, title: 'Risk scoring model: internal model v3', ref: 'Rule FIN-R-014', detail: '"Use internal scoring, not agency ratings alone"', link: 'knowledge://finance/credit/rules/FIN-R-014' },
              { n: 2, title: 'Exposure limit: under 5% portfolio per issuer', ref: 'Invariant FIN-I-002', detail: '', link: 'knowledge://finance/invariants/FIN-I-002' },
              { n: 3, title: 'Exception: reduced margin for client X', ref: 'Override FIN-O-011', detail: 'approved by head of credit | Expires: 2025-12-01', link: 'knowledge://finance/credit/overrides/FIN-O-011' },
            ],
            tradeoffsTitle: '',
            tradeoffs: [],
          },
        ],
        closing: 'Different domains. Same registry. Same resolve. Same traceability.',
      },
      importIt: {
        title: 'You don\'t have to write it. Import it.',
        body: 'Most teams won\'t manually document hundreds of decisions. They shouldn\'t have to. Knowledge populates through its API - any agent or script can propose draft entries from existing sources. A code snapshot agent is included: it scans your repos, identifies architectural patterns, technology choices, and implicit constraints, and submits them as drafts. You validate. Knowledge is populated.',
        closing: 'Your team\'s job is to validate decisions, not to write them.',
      },
      graph: {
        title: 'The graph',
        body: 'Knowledge doesn\'t store rules as a flat list. It structures them as a connected graph.',
        body2: 'Every rule links to the decision that justifies it (DEPENDS_ON). Every override links to the rule it exempts. New decisions supersede old ones with history preserved (REPLACES). Contradictions between norms are surfaced (IN_TENSION_WITH).',
        examples: '"Why does this rule exist?" - follow the graph to the decision. "What breaks if I change this?" - see every dependent rule, override, and scope affected. "Where are we inconsistent?" - tensions surface automatically.',
        closing: 'Policy engines execute rules. Knowledge connects them.',
      },
      verifierSection: {
        title: 'For engineering teams: the Verifier',
        intro: 'Engineering teams get an additional layer. Knowledge Verifier evaluates every Pull Request against the declared normative state in CI.',
        pr: {
          id: 'PR #847',
          name: 'payment-notification-service',
          scope: 'engineering/payments',
          summary: 'Resolved: 3 rules, 2 invariants, 1 active override',
          verdict: 'PASS',
        },
        items: [
          { type: 'Rule', ref: 'ENG-R-042', name: 'gRPC', status: 'OVERRIDE ACTIVE', detail: 'Override ENG-O-047: REST permitted, latency constraint', meta: 'Approved by: @cto | Expires: 2025-06-01' },
          { type: 'Invariant', ref: 'ENG-I-003', name: 'No AGPL', status: 'PASS' },
          { type: 'Invariant', ref: 'ENG-I-007', name: 'PII encrypted', status: 'PASS' },
        ],
        modeText: 'You choose the mode: report-only, fail on blocking violations, or strict with citation enforcement.',
        closing: 'The Verifier is an add-on for git/CI workflows. The core - resolve, check, query - works for any agent in any domain.',
      },
      differentByDesign: {
        title: 'Different by design',
        body: 'Knowledge is not a code scanner, a policy engine, a compliance checklist, or a prompt.',
        body2: 'Those tools evaluate artifacts or execute rules. Knowledge structures the decisions behind them.',
        closing: 'It operates at the normative layer - where humans decide what should be true, and agents need to know what has been decided.',
      },
      whyNow: {
        title: 'Why now',
        body: 'Before AI, inconsistency spread slowly. A wrong convention took months to propagate.',
        body2: 'Now it propagates instantly. Agents replicate patterns across repositories and workflows. Contradictions multiply. Exceptions accumulate.',
        body3: 'Without structured context, velocity becomes entropy.',
        closing: 'Knowledge maintains coherence at scale.',
      },
      who: {
        title: 'Who this is for',
        body: 'Engineering teams using AI coding tools. Regulated organizations deploying AI agents in production - finance, health, insurance. Platform and security teams defining invariants. CTOs who want velocity without organizational drift. And organizations that haven\'t adopted AI agents yet - because the governance framework didn\'t exist.',
      },
      interfaces: {
        title: 'Interfaces',
        items: [
          { label: 'MCP Server', description: 'Any compatible agent connects natively. Query, check, resolve, record.' },
          { label: 'REST API', description: 'CI/CD, scripts, production systems, internal workflows.' },
          { label: 'Web UI', description: 'Administration, graph visualization, impact analysis.' },
          { label: 'Slack Bot', description: 'Capture decisions where teams already work.' },
        ],
      },
      humanReadable: {
      ...
      evidenceNote: {
        ...
        p2: 'Knowledge governs operational decisions. Evidence governs immutable commitments.',
  ...
  fr: {
    ...
    nav: {
      ...
      aiAgents: 'Agents IA',
      ciVerifier: 'Vérification CI',
      gettingStarted: 'Démarrer',
      concepts: 'Concepts',
      integrations: 'Intégrations',
      docsAiAgents: 'Agents IA & MCP',
    ...
    index: {
      hero: {
        ...
        headlineItalic: 'Vos agents ne les connaissent pas.',
      ...
      heroCard: {
        title: 'Agent - Finance / Risk',
        steps: [
          { icon: '📋', label: 'Invariants chargés', text: 'scope: "finance" / ns: "risk"', badge: 'Max 5% par émetteur', badgeColor: 'blue' },
          { icon: '📐', label: 'Règles chargées', text: 'scope: "finance" / ns: "pricing"', badge: 'Benchmark EUR mid-swap — OBLIGATOIRE', badgeColor: 'blue' },
          { icon: '🔍', label: 'Vérification exposition', text: 'Actuelle : 4,2% + demandé +2% | Résultante : 6,2% > limite 5%', badge: '', badgeColor: 'orange' },
          { icon: '⚠️', label: 'Décision agent', text: 'Achat non recommandé', badge: 'Invariant finance/risk consulté', badgeColor: 'red' },
          { icon: '✓', label: 'Choix documenté', text: '', badge: 'dec-d2c9e8e8fa1b', badgeColor: 'green' },
        ],
      },
      problem: {
        tag: 'Le problème',
        title: 'Vous déployez des agents IA. Comment connaissent-ils vos règles ?',
        items: [
          'Les règles métier sont éparpillées entre de multiples sources (PDFs, e-mails, wikis, prompts).',
          'Il n\'existe pas de source de vérité unique que vos agents peuvent interroger.',
          'Chaque changement de règle impose des mises à jour manuelles sur tous vos agents.',
          'Aucune piste d\'audit ne lie les décisions des agents aux règles en vigueur au moment des faits.',
        ],
      },
      levels: {
        tag: 'Comment ça marche',
        title: 'Voir. Formaliser. Appliquer.',
        ...
        level1: {
          ...
          title: 'Knowledge extrait les décisions implicites de vos repos, PRs et docs.',
          desc: 'Knowledge analyse vos repos et génère des drafts typés : invariants, règles et décisions candidats. Rien n\'entre dans le registre sans validation humaine.',
          features: [
            'Vos propres sources via API de stream',
            'Score de confiance sur chaque draft',
            'Rien n\'entre dans le registre sans validation humaine',
            'Re-run en CI - le registre suit l\'évolution de vos pratiques',
          ],
          quote: 'La connaissance tacite ne disparaît pas — elle entre dans le registre.',
          deployment: 'Pipeline CI, lancé manuellement ou à intervalle régulier. Zéro friction développeur.',
          persona: 'CTO, équipe produit, compliance officers. La valeur remonte sans friction descendante.',
          illustration: 'Repos Git ─┐\nPRs        ─┤\nNotion     ─┤──→ Distill (LLM) ──→ Drafts typés ──→ Validation humaine ──→ Registre\nExcel      ─┤                        • Invariants candidats\nPDFs       ─┘                        • Règles candidates\n                                     • Décisions candidates\n                                     • Relations détectées',
        },
        level2: {
          ...
          concepts: {
            cards: [
              ...
              { n: 2, badge: 'Règles', badgeColor: 'blue', title: 'Directives actives', desc: 'Conventions opérationnelles mandatory ou advisory. Versionnées : quand une règle évolue, l\'historique complet est conservé.', example: 'Ex : "Utiliser la courbe EUR mid-swap pour le pricing obligataire."' },
              { n: 3, badge: 'Décisions', badgeColor: 'green', title: 'Choix documentés', desc: 'Enregistrements immutables de choix significatifs — avec contexte, raisonnement, auteur et horodatage. La raison d\'être de chaque règle.', example: 'Ex : "EUR mid-swap adopté comme benchmark obligataire — meilleure liquidité pour les spreads corporate. Approuvé par head of pricing, mars 2024."' },
              { n: 4, badge: 'Dérogations (Overrides)', badgeColor: 'orange', title: 'Exceptions formelles', desc: 'Exception documentée à une règle ou un invariant — avec approbateur nommé, justification, conditions et expiration. Pas un contournement : une exception gouvernée.', example: 'Ex : "Dérogation 30 jours — limite d\'exposition portée à 7% pour TotalEnergies, approuvée par le CIO."' },
            ],
          },
          scopes: {
            tag: 'Scopes & Namespaces',
            title: 'Organisez vos règles à la granularité de votre organisation',
            ...
            code: 'finance\n ├─ pricing      → benchmarks, courbes\n ├─ risk         → limites, VaR, exposition\n ├─ compliance   → réglementaire, reporting\n └─ operations   → validations, process\n\nengineering\n ├─ security      → secrets, accès, CVE\n ├─ architecture  → patterns, décisions\n └─ cicd          → déploiement, gates',
          },
          graphSection: {
            tag: 'Graphe de dépendances',
            title: 'Naviguez d\'une règle à la décision qui la justifie',
            ...
            relations: [
              { from: 'Rule', rel: 'DEPENDS_ON', to: 'quelle décision justifie cette directive ?' },
              { from: 'Override', rel: 'OVERRIDES', to: 'quelle exception suspend cette directive, quand et pourquoi ?' },
              { from: 'Decision', rel: 'REPLACES', to: 'ce choix en remplace-t-il un précédent ?' },
              { from: 'Rule', rel: 'IN_TENSION_WITH', to: 'ces deux directives se contredisent-elles ?' },
            ],
          },
          features: [
            'Scopes & namespaces — organisez les règles à la granularité de votre organisation',
            'Historique versionné — chaque modification est traçable, chaque évolution est conservée',
            'Consultable à tout moment — structuré, pas éparpillé',
          ],
          quote: 'Les règles ne sont plus dans la tête de quelqu\'un — elles sont dans le registre, consultables, opposables.',
        },
        level3: {
          badge: 'Niveau 3 — Appliquer',
          tag: 'Application agentique',
          title: 'Vos agents consultent Knowledge avant d\'agir',
          desc: 'Un system prompt minimal dit à l\'agent de consulter Knowledge avant d\'agir. Les règles arrivent à la demande — pas de surcharge du prompt, pas de perte en contexte long.',
          features: [
            'API REST, compatible tout LLM et tout framework',
            'MCP natif : Claude Teams, Enterprise, Copilot Studio',
            'Hooks Claude Code : UserPromptSubmit, PostToolUse, Stop',
            'GitHub Actions : gate de conformité en CI',
          ],
          quote: 'La gouvernance n\'est pas une suggestion — c\'est un registre que l\'agent consulte avant chaque action significative.',
          hooks: {
            ...
            rows: [
              { hook: 'UserPromptSubmit', trigger: 'Chaque prompt utilisateur', action: 'Injecte list_invariants + list_rules frais', effect: 'L\'agent connaît les règles à chaque interaction — pas seulement au début de la session' },
              { hook: 'PostToolUse (async)', trigger: 'Après chaque appel d\'outil', action: 'Appelle record_reference automatiquement', effect: 'La traçabilité est déterministe — indépendante du LLM' },
              { hook: 'Stop', trigger: 'Fin de session', action: 'Appelle record pour documenter le choix final', effect: 'Chaque session produit une décision structurée' },
            ],
            result: 'La gouvernance n\'est pas une suggestion dans le system prompt — c\'est un mécanisme injecté dans le runtime de l\'agent.',
          },
          flow: 'Utilisateur envoie un prompt\n    │\n    ▼\nHook UserPromptSubmit\n    → list_invariants(scope) — contraintes fraîches\n    → list_rules(scope) — directives fraîches\n    │\n    ▼\nAgent planifie son action\n    │\n    ├── Bloqué par invariant → STOP, rapport\n    ├── Requiert approbation → DEMANDE, ATTEND\n    └── Autorisé → EXÉCUTE\n    │\n    ▼\nHook PostToolUse (async)\n    → record_reference(entry_id, status="followed")\n    │\n    ▼\nHook Stop\n    → record(decision, context, reasoning)\n    │\n    ▼\nTrail complet : règles consultées → action exécutée → décision documentée',
          integrations: [
            { icon: '🤖', name: 'Claude (MCP)', desc: 'Claude Teams & Enterprise', level: 'Hooks + MCP — imposition complète' },
            { icon: '💻', name: 'Claude Code', desc: 'Agent de coding autonome', level: 'Hooks natifs — imposition par défaut' },
            { icon: '⚙️', name: 'GitHub Actions', desc: 'CI/CD — Knowledge Verifier', level: 'Gate bloquant — pas de merge sans conformité' },
            { icon: '🔌', name: 'API REST', desc: 'Tout LLM, tout framework', level: 'Consultation API — imposition à intégrer' },
          ],
          code: '# 1. Consulter les invariants — scope + namespace ciblés\ninv = knowledge.list_invariants(\n    scope="finance", namespace="risk"\n)\n\n# 2. Consulter les règles de pricing\nrules = knowledge.list_rules(\n    scope="finance", namespace="pricing"\n)\n\n# 3. Documenter le choix avec contexte et raisonnement\nknowledge.record(\n    scope="finance",\n    decision="Achat TotalEnergies non recommandé",\n    context="Demande +2%, résultante 6.2%",\n    reasoning="Invariant finance/risk max 5% consulté"\n)\n\n# 4. Tracer la référence à l\'invariant\nknowledge.record_reference(\n    entry_id=inv_id,\n    status="followed",\n    context_type="agent_session"\n)',
        },
      },
      useCases: {
        tag: 'Cas d\'usage',
        title: 'Pour tous vos agents et copilotes',
        ...
        items: [
          { type: 'Agent autonome', icon: '🏦', title: 'Finance & Trading', desc: 'Limites d\'exposition, règles de conformité, pricing — chaque décision liée à la règle qui l\'a imposée.' },
          { type: 'Agent autonome', icon: '💻', title: 'Agents de coding', desc: 'Claude Code et vos pipelines CI/CD partagent le même registre d\'architecture et de sécurité.' },
          { type: 'Agent autonome', icon: '⚙️', title: 'Pipelines CI/CD', desc: 'Aucun merge sans conformité — le Verifier bloque les PRs non conformes automatiquement.' },
          { type: 'Agent autonome', icon: '💬', title: 'Assistants conversationnels', desc: 'Règles de communication, confidentialité et conformité appliquées via MCP — sans prompt à maintenir.' },
          { type: 'Copilote humain', icon: '🏥', title: 'Santé & Pharma', desc: 'Invariants médicaux imposés au copilote. La décision reste humaine — Knowledge trace ce qui a été consulté.' },
          { type: 'Copilote humain', icon: '⚖️', title: 'Juridique & Contractuel', desc: 'Clauses obligatoires, plafonds, niveaux d\'approbation — conformité tracée à chaque interaction.' },
        ],
      },
      comparison: {
        tag: 'Pourquoi Knowledge',
        title: 'Au-delà du system prompt',
        ...
        col2: 'Knowledge',
        rows: [
          { criterion: 'Mise à jour', before: 'Manuelle, par instance', after: 'Une règle modifiée, tous les agents mis à jour' },
          { criterion: 'Application', before: 'Suggestion oubliable', after: 'Hook injecté — indépendant du LLM' },
          { criterion: 'Mémoire contextuelle', before: 'Oublié en session longue', after: 'Réinjecté à chaque prompt' },
          { criterion: 'Exceptions', before: 'Pas de mécanisme formel', after: 'Override documenté, approuvé, expirable' },
          { criterion: 'Preuve de conformité', before: 'Intention déclarée', after: 'Exécution tracée et auditable' },
        ],
      },
      aiAct: {
        tag: 'Conformité',
        title: 'Conçu pour l\'AI Act européen',
        ...
        cards: [
          { article: 'Art. 9', title: 'Gestion des risques', desc: 'Invariants codifiés. Distill détecte l\'obsolescence.' },
          { article: 'Art. 11', title: 'Documentation technique', desc: 'Décisions immutables avec contexte et raisonnement.' },
          { article: 'Art. 12', title: 'Tenue de registres', desc: 'Logging automatique de chaque interaction agent.' },
          { article: 'Art. 13', title: 'Transparence', desc: 'Registre versionné, consultable à tout point dans le temps.' },
          { article: 'Art. 14', title: 'Supervision humaine', desc: 'Approbations, dérogations formelles, rôles et permissions.' },
          { article: 'Art. 15', title: 'Robustesse', desc: 'Application via hooks — indépendante du LLM.' },
        ],
        cta: 'En savoir plus sur la conformité AI Act',
        ctaHref: '/compliance/ai-act',
      },
      integrations: {
        tag: 'Intégrations',
        title: 'Prêt en quelques lignes',
        sub: 'Knowledge s\'intègre via MCP ou API REST dans n\'importe quel LLM ou agent IA.',
        items: [
          { icon: '🤖', name: 'Claude (MCP)', desc: 'Claude Teams & Enterprise' },
          { icon: '💻', name: 'Claude Code', desc: 'Agent de coding autonome' },
          { icon: '⚙️', name: 'GitHub Actions', desc: 'Pipelines CI/CD' },
          { icon: '🔌', name: 'API REST', desc: 'Tout LLM, tout framework' },
        ],
        code: '# 1. Consulter les invariants - scope + namespace ciblés\ninv = knowledge.list_invariants(\n    scope="finance", namespace="risk"\n)\n\n# 2. Consulter les règles de pricing\nrules = knowledge.list_rules(\n    scope="finance", namespace="pricing"\n)\n\n# 3. Documenter le choix avec contexte et raisonnement\nknowledge.record(\n    scope="finance",\n    decision="Achat TotalEnergies non recommandé",\n    context="Demande +2%, résultante 6.2%",\n    reasoning="Invariant finance/risk max 5% consulté"\n)\n\n# 4. Tracer la référence à l\'invariant\nknowledge.record_reference(\n    entry_id=inv_id,\n    status="followed",\n    context_type="agent_session"\n)',
      },
      pricing: {
        tag: 'Tarifs',
        title: 'Branchez votre agent dès le premier jour.',
        sub: 'Le registre est accessible dès le plan gratuit. L\'extraction est une feature, pas un tier.',
        tiers: [
          {
            name: 'Starter',
            tagline: 'Gratuit',
            price: 'Gratuit',
            desc: 'Le registre dès le premier jour',
            features: [
              'MCP server + API REST — branchez n\'importe quel agent',
              'resolve(), check(), query() — accès runtime complet',
              '1 scope / 3 namespaces',
              'Invariants, Règles, Décisions, Dérogations',
              'Extraction Distill — one-shot',
              '3 utilisateurs',
              'Communauté (GitHub)',
            ],
          ...
          },
          {
            ...
            tagline: '299 €/mois',
            price: '299 €/mois',
            desc: 'Étendez à toute votre organisation',
            base: 'Tout le plan Starter, plus :',
            features: [
              ...
              'Re-run Distill — détection d\'obsolescence',
          ...
          },
          {
            ...
            tagline: 'Sur devis',
            price: 'Sur devis',
            desc: 'Gouvernance bout en bout à grande échelle',
            base: 'Tout le plan Team, plus :',
            features: [
              'Multi-tenant (holding → filiales)',
              'SSO & SCIM',
              'Déploiement on-premise',
              'SLA garanti',
              'Support AI Act et audit tiers',
      ...
      observability: {
        ...
        title: 'Voyez comment vos agents utilisent vos règles',
        sub: 'Bientôt — un dashboard de conformité agentique : taux followed/diverged, règles jamais consultées, signaux d\'obsolescence Distill.',
      },
      ctaFinal: {
        tag: 'Prêt à démarrer ?',
        title: 'Vos règles existent déjà. Il est temps que vos agents les connaissent.',
        body: 'Commencez par Distill — rendez visible ce qui est tacite. Puis formalisez dans un registre vivant. Puis imposez à chaque action de vos agents.',
      ...
      shift: {
        ...
        intro: 'Les agents IA modifient désormais les systèmes à la vitesse machine. Ils introduisent des dépendances, modifient des API, refactorisent l\'architecture, déploient de l\'infrastructure, évaluent des instruments financiers, analysent des données, exécutent des workflows.',
        intro2: 'Mais ils ne connaissent pas vos décisions d\'architecture, vos contraintes de conformité, vos seuils de risque, les exceptions approuvées, ni ce qui ne doit jamais changer.',
        conclusion: 'Sans couche de décision déclarée, les agents improvisent.',
      },
      knowledge: {
        title: 'Knowledge : la couche normative que vos agents interrogent avant d\'agir',
        intro: 'Knowledge est un système d\'enregistrement déclaratif pour les décisions opérationnelles. Quatre types d\'objets :',
        entities: [
          { name: 'Invariants', description: 'contraintes qui ne doivent jamais être transgressées' },
          { name: 'Rules', description: 'directives qui guident l\'implémentation et les opérations' },
          { name: 'Decisions', description: 'choix documentés avec leur raisonnement' },
          { name: 'Overrides', description: 'exceptions gouvernées, limitées dans le temps, avec justification et approbateur' },
        ],
        after1: 'Tout agent appelle `knowledge.resolve()` pour son scope et reçoit les règles applicables, les overrides actifs, les conflits détectés et la chaîne de résolution complète. Avant d\'agir, pas après.',
        after2: 'Knowledge ne bloque pas l\'exécution. Il expose l\'état décisionnel applicable. Votre écosystème décide quoi en faire.',
      },
      mcpNative: {
        title: 'Natif MCP. Dynamique par conception.',
        body: 'Les agents appellent Knowledge à la demande via MCP (Model Context Protocol).',
        body2: 'Pas d\'injection dans chaque prompt. Aucun surcoût en tokens. Aucun prompt système surchargé. Six outils dédiés, appelés uniquement quand l\'agent a besoin de contexte.',
        examples: 'Un agent engineering appelle `resolve()` avant de coder. Un agent conformité appelle `check()` avant de traiter un dossier. Un agent pricing appelle `query()` pour savoir quel modèle appliquer. Même registre, même protocole, tout workflow.',
      },
      scopeResolution: {
        title: 'Scope et résolution',
        intro: 'Le contexte se résout de manière déterministe.',
        code: 'engineering\n ├─ payments\n │    ├─ pci\n │    ├─ fraud\n ├─ platform\n ├─ data\n\nfinance\n ├─ credit\n │    ├─ investment-grade\n ├─ pricing\n ├─ risk',
        after: 'Chaque agent est mappé à un chemin unique. La résolution va du plus spécifique au plus global. Quand un agent interroge `engineering/payments/pci` ou `finance/credit/investment-grade`, Knowledge retourne uniquement les règles applicables à ce chemin.',
        closing: 'Aucune ambiguïté. Aucune précédence cachée. Aucune duplication.',
      },
      whatItLooksLike: {
        title: 'Voici à quoi ça ressemble',
        examples: [
          {
            intro: 'Un agent engineering ouvre la PR #847. Au lieu de coder à l\'aveugle, il a d\'abord consulté Knowledge.',
            header: 'Résumé de PR - Généré par l\'IA',
            what: 'Implémente le nouveau service de notification de paiement.',
            decisionsTitle: 'Décisions de conception',
            decisions: [
              { n: 1, title: 'gRPC pour la communication inter-services', ref: 'Rule ENG-R-042', detail: '"Tous les nouveaux services utilisent gRPC"', link: 'knowledge://engineering/rules/ENG-R-042' },
              { n: 2, title: 'PostgreSQL pour le stockage transactionnel', ref: 'Decision ENG-D-118', detail: '"PostgreSQL plutôt que MongoDB pour tout service nécessitant des transactions ACID"', link: 'knowledge://engineering/decisions/ENG-D-118' },
              { n: 3, title: 'Aucune dépendance AGPL utilisée', ref: 'Invariant ENG-I-003', detail: '', link: 'knowledge://engineering/invariants/ENG-I-003' },
            ],
            tradeoffsTitle: 'Compromis',
            tradeoffs: [
              { title: 'REST utilisé au lieu de gRPC pour les notifications (latence < 10ms)', ref: 'Override ENG-R-042', detail: 'approuvé par @cto', link: 'knowledge://engineering/overrides/ENG-O-047' },
            ],
          },
          {
            intro: 'Un agent conformité traite une demande de prêt. Il a d\'abord consulté Knowledge.',
            header: 'Journal d\'action de l\'agent',
            what: 'Demande de prêt #2891 - Ligne de crédit PME',
            decisionsTitle: 'Vérifié avant traitement :',
            decisions: [
              { n: 1, title: 'Modèle de scoring de risque : modèle interne v3', ref: 'Rule FIN-R-014', detail: '"Utiliser le scoring interne, pas les notations agences seules"', link: 'knowledge://finance/credit/rules/FIN-R-014' },
              { n: 2, title: 'Limite d\'exposition : moins de 5% du portefeuille par émetteur', ref: 'Invariant FIN-I-002', detail: '', link: 'knowledge://finance/invariants/FIN-I-002' },
              { n: 3, title: 'Exception : marge réduite pour le client X', ref: 'Override FIN-O-011', detail: 'approuvé par head of credit | Expire : 2025-12-01', link: 'knowledge://finance/credit/overrides/FIN-O-011' },
            ],
            tradeoffsTitle: '',
            tradeoffs: [],
          },
        ],
        closing: 'Domaines différents. Même registre. Même résolution. Même traçabilité.',
      },
      importIt: {
        title: 'Pas besoin de l\'écrire. Importez.',
        body: 'La plupart des équipes ne documenteront pas manuellement des centaines de décisions. Elles ne devraient pas avoir à le faire. Knowledge se peuple via son API - tout agent ou script peut proposer des entrées draft à partir de sources existantes. Un agent de scan de code est inclus : il analyse vos dépôts, identifie les patterns d\'architecture, les choix technologiques et les contraintes implicites, et les soumet comme drafts. Vous validez. Knowledge est peuplé.',
        closing: 'Le travail de votre équipe est de valider les décisions, pas de les rédiger.',
      },
      graph: {
        title: 'Le graphe',
        body: 'Knowledge ne stocke pas les règles comme une liste plate. Il les structure comme un graphe connecté.',
        body2: 'Chaque règle est liée à la décision qui la justifie (DEPENDS_ON). Chaque override est lié à la règle qu\'il exempte. Les nouvelles décisions remplacent les anciennes avec l\'historique préservé (REPLACES). Les contradictions entre normes sont signalées (IN_TENSION_WITH).',
        examples: '"Pourquoi cette règle existe ?" - suivez le graphe jusqu\'à la décision. "Qu\'est-ce qui casse si je change ça ?" - voyez chaque règle dépendante, chaque override et chaque scope affecté. "Où sommes-nous incohérents ?" - les tensions remontent automatiquement.',
        closing: 'Les moteurs de politiques exécutent des règles. Knowledge les connecte.',
      },
      verifierSection: {
        title: 'Pour les équipes engineering : le Verifier',
        intro: 'Les équipes engineering disposent d\'une couche supplémentaire. Knowledge Verifier évalue chaque Pull Request contre l\'état normatif déclaré en CI.',
        pr: {
          id: 'PR #847',
          name: 'payment-notification-service',
          scope: 'engineering/payments',
          summary: 'Résolu : 3 rules, 2 invariants, 1 override actif',
          verdict: 'PASS',
        },
        items: [
          { type: 'Rule', ref: 'ENG-R-042', name: 'gRPC', status: 'OVERRIDE ACTIF', detail: 'Override ENG-O-047 : REST autorisé, contrainte de latence', meta: 'Approuvé par : @cto | Expire : 2025-06-01' },
          { type: 'Invariant', ref: 'ENG-I-003', name: 'Pas d\'AGPL', status: 'PASS' },
          { type: 'Invariant', ref: 'ENG-I-007', name: 'PII chiffré', status: 'PASS' },
        ],
        modeText: 'Vous choisissez le mode : rapport uniquement, échec sur violations bloquantes, ou strict avec citations obligatoires.',
        closing: 'Le Verifier est un add-on pour les workflows git/CI. Le coeur - resolve, check, query - fonctionne pour tout agent dans tout domaine.',
      },
      differentByDesign: {
        title: 'Différent par conception',
        body: 'Knowledge n\'est pas un scanner de code, un moteur de politiques, une checklist de conformité, ni un prompt.',
        body2: 'Ces outils évaluent des artefacts ou exécutent des règles. Knowledge structure les décisions qui les sous-tendent.',
        closing: 'Il opère au niveau de la couche normative - là où les humains décident ce qui doit être vrai, et où les agents ont besoin de savoir ce qui a été décidé.',
      },
      whyNow: {
        title: 'Pourquoi maintenant',
        body: 'Avant l\'IA, l\'incohérence se propageait lentement. Une mauvaise convention mettait des mois à traverser une base de code.',
        body2: 'Aujourd\'hui, elle se propage instantanément. Les agents répliquent des patterns à travers les dépôts et les workflows. Les contradictions se multiplient. Les exceptions s\'accumulent.',
        body3: 'Sans contexte structuré, la vélocité devient de l\'entropie.',
        closing: 'Knowledge maintient la cohérence à l\'échelle.',
      },
      who: {
        title: 'À qui ça s\'adresse',
        body: 'Équipes engineering utilisant des outils de codage IA. Organisations réglementées déployant des agents IA en production - finance, santé, assurance. Équipes plateforme et sécurité définissant des invariants. CTO souhaitant de la vélocité sans dérive organisationnelle. Et organisations qui n\'ont pas encore adopté les agents IA - parce que le cadre de gouvernance n\'existait pas.',
      },
      interfaces: {
        title: 'Interfaces',
        items: [
          { label: 'Serveur MCP', description: 'Tout agent compatible se connecte nativement. Query, check, resolve, record.' },
          { label: 'API REST', description: 'CI/CD, scripts, systèmes de production, workflows internes.' },
          { label: 'Interface web', description: 'Administration, visualisation du graphe, analyse d\'impact.' },
          { label: 'Bot Slack', description: 'Capturez les décisions là où les équipes travaillent déjà.' },
        ],
      },
      humanReadable: {
      ...
      evidenceNote: {
        ...
        p2: 'Knowledge gouverne les décisions opérationnelles. Evidence gouverne les engagements immuables.',
      ...
      ctas: {
        ...
        note: 'Plan gratuit disponible. Sans carte bancaire.',
